...
(function(){
	...
	var json2markup = function(json,lvl){
		...
		if(typeof json == 'object' && json instanceof Array){
			...
			if(json.length == 0) markup.push('<div class="empty-array-wrapper lvl-"'+lvl+'">[ ]</div>');
				...
				// if empty
				for(var i=0,item;item = json[i++];) markup.push('<li class="array-item"><a href="#" class="list-toggle-button"><span class="ui-icon ui-icon-triangle-1-s" style="padding-right: 2px; width:16px; height:14px;"></span></a>'+(typeof item == 'string' || typeof item == 'number' ? '<p class="string-value">'+item+'</p>' : json2markup(item,lvl))+'</li>');
				markup.push('</ol></div>');
			}
			...
			for(var k in json){
				markup.push('<li class="object-property"><dl class="property-definition"><dt class="property-name">'+k+':<a href="#" class="property-toggle-button"><span class="ui-icon ui-icon-triangle-1-s" style="padding-right: 2px; width:16px; height:14px;"></span></a></dt><dd class="property-value">'+json2markup(json[k],lvl)+'</dd></dl></li>')
			};
			if(markup.length == ml){
			  // rewind
			  markup.shift();
			...
			// handle string
			json = $('<div style="display:  inline"/>').text(json).html();
			markup.push('<span class="string-value">'+json+'</span>');
		}else if(typeof json === 'number'){
			// handle numbers
			markup.push('<span class="string-value">'+json.toString()+'</span>');
		}else{ 
		
			// handle null
			markup.push('<span class="null-value">null</span>');
		}
		return markup.join('');
	}
	...
	$('.list-toggle-button').live('click', function(e){
		...
		if($this.data('div').is(':visible')){
			$this.html('<span class="ui-icon ui-icon-triangle-1-e" style="padding-right: 2px; width:16px; height:14px;"></span>');
			$this.data('div').hide(0,function(){
				$this.data('li').addClass('closed-'+$this.data('type'));

			});	
		}else{
			$this.html('<span class="ui-icon ui-icon-triangle-1-s" style="padding-right: 2px; width:16px; height:14px;"></span>');
